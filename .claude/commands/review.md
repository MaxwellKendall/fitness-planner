...
Analyze recent session logs, update profile.json, and produce a structured progress report. Output is HTML.

## Steps

1. **Find new sessions**
   - List all files in `session-logs/`
   - Skip dates already in `profile.json.seen_sessions`
   - Skip sessions with date ≤ `profile.json.generated_at`
...
3. **Recompute rolling volume**
   ...
   - Update `profile.json.movement_patterns.*.recent_sets_28d`
...
4. **Update strength estimates**
   ...
   - Update `profile.json.strength_levels` only if new estimate EXCEEDS current (never regress)
   - Detect PRs: new estimate > any prior → append to `profile.json.prs`
...
5. **Evaluate recovery state**
   ...
   - Otherwise: `moderate`
   - Update `profile.json.recovery_state`

6. **Evaluate goal progress**
   - Compare `profile.json.strength_levels` against age-bracket tier targets
...
7. **Goal recalibration (every 4 weeks)**
   ...
   - All recalibrations are proposals — confirm with user before updating profile.json
...
8. **Synthesize patterns**
   ...
   - Append observations to `profile.json.patterns` with date prefix
